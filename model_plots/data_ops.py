...
def broadcast(
    ...
    """
    #print("broadcast out ", xr.broadcast(xr.Dataset({x: ds1, p: ds2}), **kwargs)[0])
...
def joint_2D(
    ...
    **kwargs,
    ...
    if use_tplot:
        figg = tplot.Figure(xlabel = "value", ylabel = "prob")
        figg.line(np.array(x), np.nan_to_num(np.array(y)))
        #figg.line(np.array(x), color = "blue")
        figg.show()
    ...
    if use_tplot:
        fig = tplot.Figure(xlabel = f"value", ylabel = f"prob")

        fig.image(np.nan_to_num(stat, copy = True).T/np.max(np.nan_to_num(stat)), cmap = "ascii")
        fig.show()
...
def marginal_from_joint(
    ...
    *,
    ...
    for p in joint.coords[parameter]:
        ...
        marginal = np.append(marginal, np.sum(_y[~np.isnan(_y)]* _x[~np.isnan(_y)]))
    if use_tplot:
        figgg = tplot.Figure(xlabel = "value", ylabel = "prob")
        figgg.line(marginal)
        figgg.show()
    ...
    if fit_gaussian:
        fig = tplot.Figure()
        fig.line(np.arange(len(marginal)), marginal)
        fig.show()

        def Gauss(x, A, mu, sigma):
            return A*np.exp(sigma*(x-mu)**2)
        ...
        popt, pcov = curve_fit(
            ...
            y_s,
            #p0=[1, 0, 1],
            #bounds=([0, -3, 0.1], [2, 3, 5])
            maxfev = 3000
        )
        print(popt)
        #popt[0] = popt[0]*y.max()
        #popt[1] = popt[1]*x.std() + x.mean()
        #popt[2] = popt[2]*x.std()
        print(popt)
        parameters, _ = curve_fit(Gauss, np.arange(len(joint.coords[parameter].values)), marginal)
        #gaussian_fit = Gauss(np.arange(len(marginal)), *parameters)
        gaussian_fit2 = Gauss(x_s, *popt)
        fig = tplot.Figure()
        #fig.line(gaussian_fit)
        fig.line(x_s, gaussian_fit2, color = "red")
        fig.show()
        return xr.Dataset(
            data_vars=dict(
                x=(["bin_idx"], joint.coords[parameter].data),
                y=(["bin_idx"], marginal),
                #gauss=(["bin_idx"], gaussian_fit)
            ),
            coords=dict(
    ...
    if fit_kde:
        #print("x", [v for v in joint.coords[parameter].values])
        if (not marginal.any()):
            print("could not fit kde due to marginal being all zeros. Returning marginal as kde fit")
            return xr.Dataset(
                data_vars=dict(
                    x=(["bin_idx"], joint.coords[parameter].data),
                    y=(["bin_idx"], marginal),
                    kde=(["bin_idx"], marginal),
                ),
                coords=dict(
                    bin_idx=(["bin_idx"], np.arange(len(joint.coords[parameter].data)))
                ),
        ...
        kde = gaussian_kde(np.array(joint.coords[parameter].values), weights = np.array(marginal))
        density = kde(joint.coords[parameter].values)
        #figgg = tplot.Figure(xlabel = "value", ylabel = "prob")
        #figgg.line(joint.coords[parameter].values, density)
        #figgg.show()
        #print([i for i in joint.coords[parameter].values])
        #print([i for i in marginal])
        return xr.Dataset(
            data_vars=dict(
                x=(["bin_idx"], joint.coords[parameter].data),
                y=(["bin_idx"], marginal),
...
def marginal(
    ...
    **kwargs,
    ...
    return marginal_from_joint(
        joint, parameter=parameter, normalize=normalize, scale_y_bins=scale_y_bins, fit_gaussian = fit_gaussian, fit_kde = fit_kde
    )
...
def marginal_from_ds(
    ...
    **kwargs,
    ...
    if len(ds[x]) == 1:
        return xr.Dataset(
                data_vars=dict(
                    x=(["bin_idx"], [ds[x].values.item(), ds[x].values.item()]),
                    y=(["bin_idx"], [ds[y].values.item(), ds[y].values.item()]),
                    kde=(["bin_idx"], [ds[y].values.item(), ds[y].values.item()]),
                    gauss=(["bin_idx"], [ds[y].values.item(), ds[y].values.item()]),
...
def kde_from_ds(
    ...
    **kwargs,
    ...
    fig = tplot.Figure()
    probs = np.array(ds[y].values)
    print(probs)
    kde = gaussian_kde(np.array(ds[x].values), weights = probs)
    x_grid = np.linspace(ds[y].values.min(), ds[y].values.max(), bins)
    density = kde(x_grid)

    fig.line(x_grid, density)
    fig.show()
    return marginal(ds[x], ds[y], bins, ranges, **kwargs)
